...
module tb_TopChipTop_mixed_spi #(parameter int Burst = 3) ();
    ...
    localparam integer TIMEOUT_CYCLES = 12000000;
    ...
    task automatic spi_write_64(
        input logic [31:0] addr,
        input logic [63:0] wdata
    );
    begin
        spi_write(addr, wdata[31:0]);
        spi_write(addr + 32'd4, wdata[63:32]);
    end
    endtask

    task automatic spi_read_64(
        input logic [31:0] addr,
        output logic [63:0] rdata
    );
    begin
        spi_read(addr, rdata[31:0]);
        spi_read(addr + 32'd4, rdata[63:32]);
    end
    endtask

    task automatic spi_write_128(
        input logic [31:0] addr,
    ...
    begin
        spi_write(mpc_cfg_addr(reg_idx), wdata);
    end
    endtask
    ...
    begin
        spi_read(mpc_cfg_addr(reg_idx), rdata);
    end
    endtask
    ...
    task automatic smoke_test_sram_spi;
        integer mem_id;
        integer slot;
        integer pass_count;
    ...
    begin
        ...
        for (mem_id = 0; mem_id < 15; mem_id = mem_id + 1) begin
            ...
            for (slot = 0; slot < 3; slot = slot + 1) begin
                ...
                if (!ok) begin
                    $fatal(1, "SPI SRAM smoke test failed on mem=%0h word=%0d", mem_id[3:0], word_addr);
                end
                pass_count = pass_count + 1;
            end
        end
        $display("%t: SPI SRAM smoke test passed, total checks=%0d", $time, pass_count);
    end
    endtask
...
task automatic d2d_mem_write_word(
    ...
    initial begin
        $fsdbDumpfile("tb_TopChipTop_mixed_spi.fsdb");
        $fsdbDumpvars(0, tb_TopChipTop_mixed_spi, "+all");
    end
    ...
    initial begin
        ...
        mkdir_ret = $system("mkdir -p verification/results/full_chip/spi");
        fd_abs_in = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_abs_input.csv", "w");
        fd_abs_out = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_abs_output.csv", "w");
        fd_trans_in = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_layout_transpose_input.csv", "w");
        fd_trans_out = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_layout_transpose_output.csv", "w");
        fd_assem_in = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_layout_assemble_input.csv", "w");
        fd_assem_out = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_layout_assemble_output.csv", "w");
        fd_reduce_add_in = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_reduce_add_input.csv", "w");
        fd_reduce_add_out = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_reduce_add_output.csv", "w");
        fd_reduce_cmp_in = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_reduce_cmp_input.csv", "w");
        fd_reduce_cmp_out = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_reduce_cmp_output.csv", "w");
        fd_gemm_in = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_gemm_input.csv", "w");
        fd_gemm_out = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_gemm_output.csv", "w");
        fd_mul_in = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_mul_input.csv", "w");
        fd_mul_out = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_mul_output.csv", "w");
        fd_add_in = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_add_input.csv", "w");
        fd_add_out = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_add_output.csv", "w");
        fd_lut_in = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_lut_input.csv", "w");
        fd_lut_out = $fopen("verification/results/full_chip/spi/tb_TopChipTop_mixed_spi_lut_output.csv", "w");
        ...
        $display("tb_TopChipTop_mixed_spi completes at %t.", $time);
        $finish;
    end

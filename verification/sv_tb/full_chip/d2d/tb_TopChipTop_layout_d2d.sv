...
module tb_TopChipTop_layout_d2d #(parameter int Burst = 3) ();
    ...
    begin
        for (cid = 0; cid < CASES; cid = cid + 1) begin
            for (w = 0; w < case_src_words[cid]; w = w + 1) begin
                d2d_mem_write_word(case_src_sram[cid], case_src_base[cid] + w, pre_src_words[cid][w]);
            end
            for (w = 0; w < case_dst_words[cid]; w = w + 1) begin
                d2d_mem_write_word(case_dst_sram[cid], case_dst_base[cid] + w, pre_dst_words[cid][w]);
            end
        end
    end
    endtask
    ...
    begin
        for (w = 0; w < case_src_words[cid]; w = w + 1) begin
            d2d_mem_read_word(case_src_sram[cid], case_src_base[cid] + w, rd_word);
            post_src_words[cid][w] = rd_word;
        end
        for (w = 0; w < case_dst_words[cid]; w = w + 1) begin
            d2d_mem_read_word(case_dst_sram[cid], case_dst_base[cid] + w, rd_word);
    ...
    begin
        cycles = 0;
        while ((pad_done_pulse_count < CASES) && (cycles < TIMEOUT_CYCLES)) begin
            @(posedge clock);
            cycles = cycles + 1;
        end

        if (pad_done_pulse_count != CASES) begin
            $fatal(1, "timeout waiting for pad_dexmpc_complete count=%0d (got %0d)",
                   CASES, pad_done_pulse_count);
        end
        ...
        if (done_count_reg != CASES) begin
            $fatal(1, "doneCount_0 mismatch: expected %0d got %0d", CASES, done_count_reg);
        end
        ...
        if (done_cmd_id != (CASES - 1)) begin
            $fatal(1, "lastDone cmd id mismatch: expected %0d got %0d", CASES - 1, done_cmd_id);
        end
        if (done_opcode != OP_DATALAYOUT) begin
            $fatal(1, "lastDone opcode mismatch: opcode=%0b", done_opcode);
        end
        if (case_mode[done_cmd_id] && (done_subop !== SUB_ASSEMBLE)) begin
            $fatal(1, "lastDone subop mismatch for case %0d (assemble)", done_cmd_id);
        end
        if (!case_mode[done_cmd_id] && (done_subop !== SUB_TRANSPOSE)) begin
            $fatal(1, "lastDone subop mismatch for case %0d (transpose)", done_cmd_id);
        end
        ...
        for (cid = 0; cid < CASES; cid = cid + 1) begin
            rsp_seen[cid] = 1'b1;
            rsp_seq[cid] = case_seq[cid];
            rsp_reqId[cid] = case_reqId[cid];
            rsp_mode[cid] = case_mode[cid];
            rsp_srcSramId[cid] = case_src_sram[cid][1:0];
            rsp_dstSramId[cid] = case_dst_sram[cid][1:0];
            rsp_srcBase[cid] = case_src_base[cid][ADDR_W-1:0];
            rsp_srcRows[cid] = case_src_rows[cid][7:0];
            rsp_srcCols[cid] = case_src_cols[cid][7:0];
            rsp_dstBase[cid] = case_dst_base[cid][ADDR_W-1:0];
            rsp_dstRows[cid] = case_dst_rows[cid][7:0];
            rsp_dstCols[cid] = case_dst_cols[cid][7:0];
            rsp_offRow[cid] = case_off_r[cid][7:0];
            rsp_offCol[cid] = case_off_c[cid][7:0];
            rsp_done[cid] = 1'b1;
            done_seen[cid] = 1'b1;
        end
    end
    ...
    initial begin
        $fsdbDumpfile("tb_TopChipTop_layout_d2d.fsdb");
        $fsdbDumpvars(0, tb_TopChipTop_layout_d2d, "+all");
    end
    ...
    initial begin
        ...
        in_csv_fd = $fopen("verification/results/full_chip/d2d/tb_TopChipTop_layout_d2d_input.csv", "w");
        if (in_csv_fd == 0) begin
            $fatal(1, "failed to open verification/results/full_chip/d2d/tb_TopChipTop_layout_d2d_input.csv");
        end
        out_csv_fd = $fopen("verification/results/full_chip/d2d/tb_TopChipTop_layout_d2d_output.csv", "w");
        if (out_csv_fd == 0) begin
            $fatal(1, "failed to open verification/results/full_chip/d2d/tb_TopChipTop_layout_d2d_output.csv");
        end
        ...
        $display("tb_TopChipTop_layout_d2d completes at %t.", $time);
        $finish;
    end

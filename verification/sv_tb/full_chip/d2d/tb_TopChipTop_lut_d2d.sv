...
module tb_TopChipTop_lut_d2d #(parameter int Burst = 3) ();
    ...
    begin
        case (core_mem_id)
            MEM_GLOBAL: core_mem_to_mpc_mem = 4'h0;
            MEM_LOCAL0: core_mem_to_mpc_mem = 4'h1;
            MEM_TEMP0: core_mem_to_mpc_mem = 4'h5;
            MEM_TRIG_SIN_EVEN: core_mem_to_mpc_mem = 4'h9;
            MEM_TRIG_SIN_ODD: core_mem_to_mpc_mem = 4'hA;
            MEM_TRIG_COS_EVEN: core_mem_to_mpc_mem = 4'hB;
            MEM_TRIG_COS_ODD: core_mem_to_mpc_mem = 4'hC;
            MEM_SOFT_EVEN: core_mem_to_mpc_mem = 4'hD;
            MEM_SOFT_ODD: core_mem_to_mpc_mem = 4'hE;
    ...
    begin
        ...
        fd = $fopen("verification/data/full_chip/all_inputs.csv", "r");
        if (fd == 0) begin
            $fatal(1, "failed to open verification/data/full_chip/all_inputs.csv");
        end
    ...
    begin
        if (addr >= sram_depth(core_mem_to_mpc_mem(mem_id))) begin
            $fatal(1, "SRAM addr overflow: mem=%0d addr=%0d depth=%0d",
                   mem_id, addr, sram_depth(core_mem_to_mpc_mem(mem_id)));
        end
    ...
    begin
        for (idx = 0; idx < TRIG_TOTAL_WORDS; idx = idx + 1) begin
            ...
            if (idx < TRIG_BANK_DEPTH) begin
                d2d_mem_write_word(MEM_TRIG_SIN_EVEN, idx, word);
            end else if (idx < (2 * TRIG_BANK_DEPTH)) begin
                d2d_mem_write_word(MEM_TRIG_SIN_ODD, idx - TRIG_BANK_DEPTH, word);
            end else if (idx < (3 * TRIG_BANK_DEPTH)) begin
                d2d_mem_write_word(MEM_TRIG_COS_EVEN, idx - (2 * TRIG_BANK_DEPTH), word);
            end else begin
                d2d_mem_write_word(MEM_TRIG_COS_ODD, idx - (3 * TRIG_BANK_DEPTH), word);
            end
        end
    end
    endtask
    ...
    begin
        for (idx = 0; idx < SOFTPLUS_TOTAL_WORDS; idx = idx + 1) begin
            ...
            if (idx < SOFTPLUS_BANK_DEPTH) begin
                d2d_mem_write_word(MEM_SOFT_EVEN, idx, word);
            end else begin
                d2d_mem_write_word(MEM_SOFT_ODD, idx - SOFTPLUS_BANK_DEPTH, word);
            end
        end
    end
    endtask
    ...
    begin
        ...
        if (case_op[issued_cmd_count - 1] == 0) begin
            expect_subop = SUB_SIN;
        end else if (case_op[issued_cmd_count - 1] == 1) begin
            expect_subop = SUB_COS;
        end else begin
            expect_subop = SUB_SOFTPLUS;
        end
        if (done_opcode != OP_LUT || done_subop != expect_subop[3:0]) begin
    ...
    initial begin
        $fsdbDumpfile("tb_TopChipTop_lut_d2d.fsdb");
        $fsdbDumpvars(0, tb_TopChipTop_lut_d2d, "+all");
    end
    ...
    initial begin
        ...
        in_csv_fd = $fopen("verification/results/full_chip/d2d/tb_TopChipTop_lut_d2d_input.csv", "w");
        if (in_csv_fd == 0) begin
            $fatal(1, "failed to open verification/results/full_chip/d2d/tb_TopChipTop_lut_d2d_input.csv");
        end
        out_csv_fd = $fopen("verification/results/full_chip/d2d/tb_TopChipTop_lut_d2d_output.csv", "w");
        if (out_csv_fd == 0) begin
            $fatal(1, "failed to open verification/results/full_chip/d2d/tb_TopChipTop_lut_d2d_output.csv");
        end
        ...
        $display("tb_TopChipTop_lut_d2d completes at %t.", $time);
        $finish;
    end

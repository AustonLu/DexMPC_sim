...
int dexsim_run_detailed(void* handle, const dexsim_command* commands, size_t command_count,
    ...
    return guard([&] {
        if (command_count != 0 && commands == nullptr) {
            throw std::runtime_error("dexsim_run_detailed received a null command pointer");
        }
        if (stats == nullptr) {
            throw std::runtime_error("dexsim_run_detailed received a null stats pointer");
        }
        if (result_count < command_count || (command_count != 0 && results == nullptr)) {
            throw std::runtime_error("dexsim_run_detailed result buffer is too small");
        }
        std::vector<dexsim::runtime::Command> batch(command_count);
        for (size_t index = 0; index < command_count; ++index) {
            for (int word = 0; word < 3; ++word) {
        ...
        if (value.command_results.size() != command_count) {
            throw std::runtime_error("dexsim_run_detailed command result count mismatch");
        }
        for (size_t index = 0; index < command_count; ++index) {
...
int dexsim_get_counters(void* handle, dexsim_counters* counters) {
    return guard([&] {
        if (counters == nullptr) {
            throw std::runtime_error("dexsim_get_counters received a null output pointer");
        }
        const auto value = session(handle).counters();
        counters->cycle = value.cycle;
        counters->read_bytes = value.read_bytes;
        counters->write_bytes = value.write_bytes;
    });
}

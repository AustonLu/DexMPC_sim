...
#
#            (C) COPYRIGHT 2001-2014 ARM Limited.
#                ALL RIGHTS RESERVED
#
...
#
# File Revision       : 180917
# File Date           :  2014-09-18 15:20:30 +0100 (Thu, 18 Sep 2014)
# Release Information : PL401-r1p2-00rel0
#-------------------------------------------------------------------------------
# Purpose : Converts an ASCII command file into a hex format, suitable for
#           import by the FileReader bus master.
#           Checks that the specified commands are compatible with the
#           FileReader.
#
#           Options:
#                -help
#                -quiet
#                -adk1
#                -buswidth=<databus width>    32, 64, 128 or 256
#                -addrwidth=<address width>   32 (default) to 64
#                -endian=<endianness>         big or little(default)
#                  Note: big-endian is BE-32 (a.k.a. ARM big-endian)
#                -stimarraysize=<size>        default = 50000 (words)
...
my $numLanesBus     = 0;  # Number of lanes determined from the specified
                          # bus width
my $maxBytesBus     = 0;  # Number of byte-lanes in the internal bus
my $maxAddr         = Math::BigInt->new(0);  # Maximum value of allowed address
...
my $Const32  = 4294967296; # 32-bit maximum integer constant

# default values for stimulus fields
$command{'LINE'}      = 0;
$command{'DIFF'}      = '000000';
$command{'CMD'}       = '00000000'; # idle
$command{'ADDR'}      = '0000000000000000';
$command{'DATA'}      = '00'x($busWidth/8);
$command{'MASK'}      = '';
$command{'SIZE'}      = '000';
$command{'BUR'}       = '000';
$command{'PROT'}      = '000000';
...
# Calculated after addrWidth is fixed
$maxAddr = Math::BigInt->new(1);
for (my $i = 1; $i < $addrWidth; $i++) {
  $maxAddr = ($maxAddr << 1) + 1;
}
...
while (<INPUTFILE>) {
  ...
  # If a character exists on this line and it is not whitespace...
    ...
    # simulation time and cannot be looped
      ...
      # Check that comments do not follow an EMIT
      if ($currCmd eq 'EMIT') {
        printf "Line %4d: ERROR 115: Comment command cannot ".
        "follow an EMIT\n", $lineNum;
        $fileErrors++;
      }

    # If line starts with any other valid command then decode command
    } elsif(s/^\s*(W|R|S|B|I|P|L|Q|QUIT|EMIT|WAIT|write|read|seq|sequential|busy|idle|poll|loop|quit|emit|wait|Quit|Emit|Wait)\b//i) {


      $prevCmd = $currCmd;  # save previous command in case of Loop commands
      if ($1 eq 'EMIT' or $1 eq 'emit' or $1 eq 'WAIT' or $1 eq 'wait' or $1 eq 'Emit' or 'Wait' or $1 eq 'QUIT' or 'Quit' or 'quit') {
      ...
      # Check that the WAIT instruction always follows an IDLE or BUSY
      if ($currCmd eq 'WAIT') {
        unless (($prevCmd eq 'B') or ($prevCmd eq 'I')) {
          printf "Line %4d: ERROR 113: WAIT command follows $prevCmd. ".
          "Can only follow IDLE or BUSY\n", $lineNum;
          $fileErrors++;
        }
      }
      ...
      if ($currCmd eq 'L') {
        if ($prevCmd eq 'EMIT') {
          printf "Line %4d: ERROR 114: Loop command cannot follow EMIT ".
          "command\n", $lineNum;
          $fileErrors++;
        }
        if ($prevCmd eq 'WAIT') {
          printf "Line %4d: ERROR 114: Loop command cannot follow WAIT ".
          "command\n", $lineNum;
          $fileErrors++;
        }
      }
      ...
      elsif ($currCmd eq 'Q' or $currCmd eq 'Quit' or $currCmd eq 'QUIT') { ParseQ() }
...
sub ParseW {
  ...
  ($command{'DATA7'}, $command{'DATA6'},$command{'DATA5'}, $command{'DATA4'}, $command{'DATA3'}, $command{'DATA2'},$command{'DATA1'}, $command{'DATA0'}, $command{'MASK7'}, $command{'MASK6'},$command{'MASK5'}, $command{'MASK4'}, $command{'MASK3'}, $command{'MASK2'}, $command{'MASK1'}, $command{'MASK0'})
    = AlignDataMask (
                      $command{'SIZE'},
                      $command{'DATA'},
                      $command{'MASK'},
...
sub ParseI {
  ...
  if (($addrIbd ne '') && $sizeDef) {
      printf "Line %4d: WARNING 166: Possible IDLE address/size confusion.".
             " Field \"%s\" found on IDLE command line: this is being".
             " treated as the address value, but may have been intended".
             " as the size value\n", $lineNum, $addrIbd;
  };
...
sub ParseEMIT {
  if ($adk1 == 1) {
    printf "Line %4d: ERROR 110: EMIT command is not compatible with ".
    "ADK1v1 stimulus format\n", $lineNum;
    $fileErrors++;
  }
  $command{'CMD'}  = '10010000';

  # Get optional ID field
  $command{'ID'}    = GetEventIDField();
}
...
sub ParseWAIT {
  if ($adk1 == 1) {
    printf "Line %4d: ERROR 110: WAIT command is not compatible with ".
    "ADK1v1 stimulus format\n", $lineNum;
    $fileErrors++;
  }
...
sub GetAddrField
{
  ...
  if (s/\b(?:0x)?([0-9A-F]{1,16})\b//i) {
    ...
    # check address is not more than maximum supported
    if ($addr > $maxAddr) {
      printf "\nLine %4d: ERROR 47: Address exceeds maximum possible for".
             " address bus width\n", $lineNum;
      $fileErrors++;
    } else {
      $addr = ToHex64($addr);
...
sub GetIdleAddrField
{
  ...
  if (s/\b0x([0-9A-F]{1,16})\b//i ) {
      $addr = FromHexL($1);
  } elsif (s/\b([0-9A-F]{1,16})\b//i ) {
      $addr = $1;
      # Store captured value if it matches b or d
      ($addr =~ m/\b[bd]\b/) and $addrIbd = $addr;
      $addr = FromHexL($addr);
  } else {
      # default
      $addrIdef = 1;
...
sub GetDataField {
  ...
  if ($adk1 == 0 and s/\b(?:0x)?([0-9A-FX]{2,64})\b//i) {
    $data = $1;
  ...
  if ($data) {
    ...
    # check data field width is either 2,4,8,16,32  or 64 hex digits
...
sub GetMaskField {
 ...
 my $maskFieldWidth  = 0;        # width of mask field read from stimulus
  ...
  if ($adk1 == 0 and s/\b(?:0x)?([0-9A-F]{2,64})\b//i ) {
    $mask = $1;
  ...
  unless ($mask eq '') {
    ...
    # check mask field width is either 2,4,8 or 16 hex digits
        ...
        ($maskFieldWidth != 8) && ($maskFieldWidth != 16) && ($maskFieldWidth != 32) && ($maskFieldWidth != 64)) {
...
sub GetSizeField {
  ...
  } elsif (s/\bsize128\b//i) {
    if (($busWidth < 128)) {
      printf "Line %4d: ERROR 40: size128 not valid".
...
sub GetWriteRespField {
  ...
  if (s/\bok\b|\bokay\b//i) {
    ...
    printf "Line %4d: ERROR 104: ERROR CANCEL response not supported\n", $lineNum;
    $fileErrors++;
    $resp = '00';
...
sub GetBstrbField {
  ...
  if (s/\b[s|S]([0|1]{1,32})\b//i) {
    ...
    # check Bstrb field width is either 1,2,4 or 8 binary digits
          ...
          ($bstrbFieldWidth != 4) && ($bstrbFieldWidth != 8) && ($bstrbFieldWidth != 16)) {
...
sub PackComment {
  ...
  @words = unpack ( 'N20', $commentString."\x00\x00\x00" );
...
sub IncAddr {
  ...
  $incrAddr &= (FromHexL("FFFFFFFFFFFFFFFF") - $sizeBytes + 1);
...
sub AlignDataMask {
  ...
  ($data7, $data6, $data5, $data4,$data3, $data2, $data1, $data0) = ShiftData( $data, $bytesData, $offset );

  # Shift the mask to the correct byte lanes
  ($mask7, $mask6, $mask5, $mask4,$mask3, $mask2, $mask1, $mask0) = ShiftData( $mask, $bytesMask, $offset );

  return ($data7, $data6, $data5, $data4, $data3, $data2, $data1, $data0, $mask7, $mask6, $mask5, $mask4, $mask3, $mask2, $mask1, $mask0);
}
...
sub ShiftData {
  ...
  if ($busWidth==256) {
    $word7 = substr $fullWidth, 0, 8;
    $word6 = substr $fullWidth, 8, 8;
    $word5 = substr $fullWidth, 16, 8;
    $word4 = substr $fullWidth, 24, 8;
    $word3 = substr $fullWidth, 32, 8;
    $word2 = substr $fullWidth, 40, 8;
    $word1 = substr $fullWidth, 48, 8;
    $word0 = substr $fullWidth, 56, 8;
  }
  elsif ($busWidth==128) {
    $word7 = "";
    $word6 = "";
    $word5 = "";
    $word4 = "";
    $word3 = substr $fullWidth, 0, 8;
    $word2 = substr $fullWidth, 8, 8;
    $word1 = substr $fullWidth, 16, 8;
    $word0 = substr $fullWidth, 24, 8;
  }
  elsif ($busWidth==64) {
    $word7 = "";
    $word6 = "";
    ...
    $word3 = "";
    $word2 = "";
    $word1 = substr $fullWidth, 0, 8;
    $word0 = substr $fullWidth, 8, 8;
  }
  else {
    $word7 = "";
    $word6 = "";
    ...
    $word2 = "";
    $word1 = "";
    $word0 = substr $fullWidth, 0, 8;
  }

  return ($word7, $word6, $word5, $word4, $word3, $word2, $word1, $word0);
}
...
sub Check1kBdy {
  ...
  if  (
    ...
    and (
          ( ($numTransfers * $transSize + $startAddr) & FromHexL('FFFFFFFFFFFFFC00') )
           != ($startAddr & FromHexL('FFFFFFFFFFFFFC00'))
        )
      ) {
...
sub CheckUnalignBstrb {
  ...
  if ($endian eq 'big') {
    ...
    if ($endOfUnaligned == 1) {
      ...
      $seqOffset = FromHexL($seqAddress) & ($maxBytesBus - 1);
...
sub CheckAddrAlign {
  ...
  if  ( $addr and (
        ( ($size eq '100') && ($addr !~ /([0-9A-Fa-f]{15})[0]/i) ) or
        ( ($size eq '011') && ($addr !~ /([0-9A-Fa-f]{15})[0|8]/i) ) or
        ( ($size eq '010') && ($addr !~ /([0-9A-Fa-f]{15})[0|4|8|C]/i) ) or
        ( ($size eq '001') && ($addr !~ /([0-9A-Fa-f]{15})[0|2|4|6|8|A|C|E|]/i) )
      ) ) {
...
my $cmd = shift;
  ...
  if ($cmd eq 'W') {
    $memorySize += (3+$busWidth/32);              # Write
    $memorySize++ if ($addrWidth > 32);

  } elsif ($cmd eq 'R') {
    $memorySize += (4+$busWidth/16);              # Read
    $memorySize++ if ($addrWidth > 32);

  } elsif ($cmd eq 'S') {
    $memorySize += (3+$busWidth/16);              # Seq
  ...
  } elsif ($cmd eq 'Quit' or $cmd eq 'Q' or $cmd eq 'QUIT') {
    $memorySize += 1;                             # Quit
...
# ------------------------------------------------------------------------------
# Converts a numeric value to 8 hex digits
#
# ------------------------------------------------------------------------------
sub ToHex32 {
  sprintf("%08X", shift);
...
# ------------------------------------------------------------------------------
# Converts a numeric value to 16 hex digits
#
# ------------------------------------------------------------------------------
sub ToHex64
{
  ...
  if ($DecValue >= $Const32) {
    $TmpValue = $DecValue % $Const32;
    $DecValue = ($DecValue - $TmpValue) / $Const32;
    $HexValue = ToHex32($DecValue).ToHex32($TmpValue);
  } else {
    $HexValue = '00000000'.ToHex32($DecValue);
  }
...
# ------------------------------------------------------------------------------
# Converts a larger than 8 hex digits string to an integer
#
# ------------------------------------------------------------------------------
sub FromHexL
{
  ...
  my $i;
  while ($length > 8) {
    $i = ($length % 8 == 0) ? 8 : ($length % 8);
    $addr += (hex substr($string, 0, $i)) * $Const32;
...
$command{'FUNCTION'}
@<<<<<<<
substr($command{'ADDR'}, 0, 8) if ($addrWidth > 32)
@<<<<<<<
substr($command{'ADDR'}, 8, 8)
@<<<<<<<
$command{'DATA7'} if ($command{'DATA7'} ne "")
@<<<<<<<
$command{'DATA6'} if ($command{'DATA6'} ne "")
@<<<<<<<
$command{'DATA5'} if ($command{'DATA5'} ne "")
@<<<<<<<
$command{'DATA4'} if ($command{'DATA4'} ne "")
@<<<<<<<
$command{'DATA3'} if ($command{'DATA3'} ne "")
@<<<<<<<
$command{'DATA2'} if ($command{'DATA2'} ne "")
@<<<<<<<
$command{'DATA1'} if ($command{'DATA1'} ne "")
@<<<<<<<
$command{'DATA0'}
@<<<<<<<
...
$command{'DATA0'}
@<<<<<<<
$command{'MASK7'} if ($command{'MASK7'} ne "")
@<<<<<<<
$command{'MASK6'} if ($command{'MASK6'} ne "")
@<<<<<<<
$command{'MASK5'} if ($command{'MASK5'} ne "")
@<<<<<<<
$command{'MASK4'} if ($command{'MASK4'} ne "")
@<<<<<<<
$command{'MASK3'} if ($command{'MASK3'} ne "")
@<<<<<<<
$command{'MASK2'} if ($command{'MASK2'} ne "")
@<<<<<<<
$command{'MASK1'} if ($command{'MASK1'} ne "")
@<<<<<<<
$command{'MASK0'}
@<<<<<<<
$command{'BSTRBHEX'} if ($command{'WRITEBS'} eq '1')

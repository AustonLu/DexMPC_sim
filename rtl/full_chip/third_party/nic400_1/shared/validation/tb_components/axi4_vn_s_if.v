...
// -----------------------------------------------------------------------------
    ...
    parameter INSTANCE_TYPE                 = "AXI4S_";
...
//------------------------------------------------------------------------------
    ...
    reg      [ID_MAX:0] WID_front0;
    reg                 WID_empty0;
    logic    [ID_MAX:0] IDS0[$];
    reg      [ID_MAX:0] WID_front1;
    reg                 WID_empty1;
    logic    [ID_MAX:0] IDS1[$];
    reg      [ID_MAX:0] WID_front2;
    reg                 WID_empty2;
    logic    [ID_MAX:0] IDS2[$];
    reg      [ID_MAX:0] WID_front3;
    reg                 WID_empty3;
    logic    [ID_MAX:0] IDS3[$];
    ...
    assign aw_new = (AWVALID & (awready_prev | ~awvalid_prev));
    ...
    always @(posedge ACLK or negedge ARESETn) begin
        if (~ARESETn) begin
           ...
           //VNET 0
           if (aw_new & AWVNET == VNET_VALUE0) IDS0.push_back(AWID);
           if (WVALID & WREADY & WLAST & WVNET == VNET_VALUE0) void'(IDS0.pop_front());        
           WID_front0 =  IDS0[0];
           WID_empty0 = (IDS0.size() == 0);

           //VNET 1
           if (aw_new & AWVNET == VNET_VALUE1) IDS1.push_back(AWID);
           if (WVALID & WREADY & WLAST & WVNET == VNET_VALUE1) void'(IDS1.pop_front());        
           WID_front1 =  IDS1[0];
           WID_empty1 = (IDS1.size() == 0);

           //VNET 2
           if (aw_new & AWVNET == VNET_VALUE2) IDS2.push_back(AWID);
           if (WVALID & WREADY & WLAST & WVNET == VNET_VALUE2) void'(IDS2.pop_front());        
           WID_front2 =  IDS2[0];
           WID_empty2 = (IDS2.size() == 0);

           //VNET 3
           if (aw_new & AWVNET == VNET_VALUE3) IDS3.push_back(AWID);
           if (WVALID & WREADY & WLAST & WVNET == VNET_VALUE3) void'(IDS3.pop_front());        
           WID_front3 =  IDS3[0];
           WID_empty3 = (IDS3.size() == 0);
    ...
    always @(*) begin
        ...
        WID = (WVNET == VNET_VALUE0) ? ((WID_empty0) ? AWID : WID_front0) :
              (WVNET == VNET_VALUE1) ? ((WID_empty1) ? AWID : WID_front1) :
              (WVNET == VNET_VALUE2) ? ((WID_empty2) ? AWID : WID_front2) :
              (WVNET == VNET_VALUE3) ? ((WID_empty3) ? AWID : WID_front3) : AWID;

        wmask = (WVNET == VNET_VALUE0) ? WID_empty0 & ~(AWVALID && AWVNET == VNET_VALUE0) :
                (WVNET == VNET_VALUE1) ? WID_empty1 & ~(AWVALID && AWVNET == VNET_VALUE1)  :
                (WVNET == VNET_VALUE2) ? WID_empty2 & ~(AWVALID && AWVNET == VNET_VALUE2)  :
                (WVNET == VNET_VALUE3) ? WID_empty3 & ~(AWVALID && AWVNET == VNET_VALUE3)  : 1'b1;
    end
  
    //Mask the wdata    
    assign WVALID_masked = WVALID & ~wmask;
    assign WREADY        = WREADY_masked & ~wmask;

    FileRdSlaveAxi4 uAxiSlave (

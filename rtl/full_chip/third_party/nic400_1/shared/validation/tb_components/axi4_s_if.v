...
//------------------------------------------------------------------------------
  ...
  defparam uAxiSlave.W_ARRAY_SIZE      = 50000;             // Size of AW channel array
  ...
  always @(posedge ACLK or negedge ARESETn) begin
      if (~ARESETn) begin
         ...
         if (aw_new) IDS.push_back(AWID);
         if (WVALID & WREADY & WLAST) void'(IDS.pop_front());        
         WID_front =  IDS[0];
         WID_empty = (IDS.size() == 0);
      end
  end

  always @(*)
      WID = (WID_empty) ? AWID : WID_front;

  //Mask the wdata    
  assign WVALID_masked = WVALID & (~WID_empty | AWVALID);
  assign WREADY        = WREADY_masked & (~WID_empty | AWVALID);
...
FileRdSlaveAxi4 uAxiSlave (
  ...
  assign AWUSER_int = {AWUSER, 8'h00, AWQOS, AWREGION};
  assign ARUSER_int = {ARUSER, 8'h00, ARQOS, ARREGION};

...
//------------------------------------------------------------------------------
  ...
  always @(posedge ACLK) begin
    aw_chan_q   <= {{ID_PAD_WIDTH{1'b0}},AWID,AWADDR,AWLEN,AWSIZE,AWBURST,AWCACHE,AWPROT,1'b0,AWLOCK,AWVALID,AWREADY};
    w_chan_q    <= {{ID_PAD_WIDTH{1'b0}},{ID_WIDTH{1'b0}},WLAST,WVALID,WREADY};
    b_chan_q    <= {{ID_PAD_WIDTH{1'b0}},BID,BRESP,BVALID,BREADY};
    ar_chan_q   <= {{ID_PAD_WIDTH{1'b0}},ARID,ARADDR,ARLEN,ARSIZE,ARBURST,ARCACHE,ARPROT,1'b0,ARLOCK,ARVALID,ARREADY};
...
//                 );
  ...
  always @(posedge ACLK)
    ...
    $fwrite(TraceFile,"%x %b %x %x %x %x %x %x %x\n",
            GlobalTimeStamp,
            ARESETn,
            {      1'b0,          AWLEN[7:4], AWPROT, 1'b0, AWLOCK, AWLEN[3:0], AWVALID, AWREADY, {ID_PAD_WIDTH{1'b0}}, AWID},
            {{13{1'b0}},                          WLAST, WVALID,  WREADY,  {ID_PAD_WIDTH{1'b0}},  WID},
            {{12{1'b0}},                          BRESP, BVALID,  BREADY,  {ID_PAD_WIDTH{1'b0}},  BID},
            {      1'b0,          ARLEN[7:4], ARPROT, 1'b0, ARLOCK, ARLEN[3:0], ARVALID, ARREADY, {ID_PAD_WIDTH{1'b0}}, ARID},

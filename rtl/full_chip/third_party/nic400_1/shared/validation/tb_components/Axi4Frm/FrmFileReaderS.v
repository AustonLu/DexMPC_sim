...
(
  ...
  output              FileValidPoll;    // Valid vector in file for polls
...
//------------------------------------------------------------------------------
  ...
  assign FileValid     = (ArrayPt < ArrayMax) ? ~SyncValid : 1'b0;
  assign FileValidPoll = (ArrayPt < ArrayMax) ? 1'b1 : 1'b0;
...
//------------------------------------------------------------------------------
  always @(Wait or ArrayPt or Wait_flags or ArrayMax or Restart) begin
      ...
      while ((!Vector[0] || Restart) && (Pointer < ArrayMax)) begin

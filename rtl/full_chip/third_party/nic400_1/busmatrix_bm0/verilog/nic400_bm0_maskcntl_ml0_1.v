...
module nic400_bm0_maskcntl_ml0_1
  (
  ...
  reg               next_comb_cnt;      // next outstanding trans count 
  ...
  reg             comb_cnt;        // outstanding trans count
  ...
  // For combined issuing of 1 only combined count needs to be compared
  ...
  always @(next_wr_cnt or next_rd_cnt or next_comb_cnt or next_full) 
   begin : p_mask_comb
     next_mask_w = {3{1'b0}};
     next_mask_r = {3{1'b0}};
     // If the next write count or next combined count will be reached then
     // mask all the write slave interfaces
     if (next_wr_cnt == 1'b1 || next_comb_cnt == 1'b1 || next_full)
        next_mask_w = {3{1'b1}};
     // If the next read count or next combined count will be reached then

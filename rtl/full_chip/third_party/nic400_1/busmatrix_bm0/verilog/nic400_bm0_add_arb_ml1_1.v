...
module nic400_bm0_add_arb_ml1_1
  (
  ...
  input             override;
  // Next required quality value
  output [3:0]      reqd_qv;    // QV value to be arbitrated next
  // Valid signal for counters
  output            valid_cnt;
  ...
  //------------- Determine if a valid will be set high this cycle ------------


  assign valid_cnt = (|(~mask & a_valid_vector) & ~override) | stall;
...
assign request = is_highest_qos & ~mask & {REQUESTS{~override}};
   ...
   assert_never #(0,0,"ERROR, No transaction was arbitrated when expected") 
      ovl_lrg_always_arb
       (
        .clk       (aclk),
        .reset_n   (aresetn),
        .test_expr ((|(a_valid_vector & ~mask) && !override) && (~|a_sel))
       );

...
#define DEXMPC_RESULT_DIR "verification/results/software_stack/device_mixed/d2d"
#endif

int main(int argc, char** argv) {
    try {
        auto device = dexmpc::runtime::Device::open_sim(argc, argv, dexmpc::runtime::Transport::D2D);
        dexmpc::tests::DeviceMixedTest test(device, std::filesystem::path(DEXMPC_RESULT_DIR), "D2D");
        test.run();
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "Device mixed D2D test failed: " << e.what() << "\n";
        return 1;
    }
}

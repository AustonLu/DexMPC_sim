...
int main(int argc, char** argv) {
    try {
        ...
        auto input = runtime.upload_matrix(src, dexsim::kMemGlobal, "op_d2d_input");
        int released_base = -1;
        {
            auto output = runtime.abs(input, dexsim::kMemLocal0, "op_d2d_abs");
            released_base = output.word_addr();
            expect_matrix_eq(runtime.download_matrix(output), expected_abs, "operator abs D2D");
        }
        auto reused = runtime.empty_matrix(dexsim::kMemLocal0, 2, 4, "op_d2d_reused");
        if (reused.word_addr() != released_base) {
            throw std::runtime_error("operator D2D result memory was not auto released and reused");
        }

        std::cout << "OperatorRuntime D2D smoke passed at cycle " << device.cycle() << "\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "OperatorRuntime D2D smoke failed: " << e.what() << "\n";
        return 1;
    }
}

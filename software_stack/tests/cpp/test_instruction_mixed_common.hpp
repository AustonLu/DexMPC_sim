...
using dexmpc::runtime::Instruction;
using dexmpc::runtime::InstructionRuntime;
using dexmpc::runtime::StatusRegisters;
using dexmpc::runtime::VariableRef;
...
public:
    DeviceMixedTest(Device& device, std::filesystem::path out_dir, std::string label)
        : runtime_(device), out_dir_(std::move(out_dir)), label_(std::move(label)) {}
    ...
    void run() {
        ...
        std::cout << "Device mixed " << label_ << " test passed at cycle " << runtime_.device().cycle()
                  << ", cases=" << num_cases_
...
private:
    ...
    std::string var_name(const MixedCase& c, const char* role) const {
        return "case_" + std::to_string(c.cmd_id) + "_" + role;
    }

    VariableRef alloc_matrix(const MixedCase& c, const char* role, int mem_id, int rows, int cols) {
        return runtime_.allocate_matrix(var_name(c, role), mem_id, rows, cols);
    }

    VariableRef alloc_vector(const MixedCase& c, const char* role, int mem_id, int len) {
        return runtime_.allocate_vector(var_name(c, role), mem_id, len);
    }

    void apply_instruction(MixedCase& c, const Instruction& instruction) {
        c.cmd_id = instruction.cmd_id;
        c.opcode = instruction.opcode;
        c.subop = instruction.subop;
        c.cmd = instruction.raw;
    ...
    void add_abs_case(int src_mem, int dst_mem, int rows, int cols, int seed) {
        ...
        const auto src_ref = alloc_matrix(c, "src", src_mem, rows, cols);
        const auto dst_ref = alloc_matrix(c, "dst", dst_mem, rows, cols);
        c.src_base = src_ref.word_addr;
        c.dst_base = dst_ref.word_addr;
        c.src_words = src_ref.word_count;
        c.dst_words = dst_ref.word_count;

        Matrix src = fill_pattern_matrix(rows, cols, seed, true);
        c.src_words = pack_matrix_words(c.pre_src, src, rows, cols);
        apply_instruction(c, runtime_.abs(src_ref.name, dst_ref.name));
    }
    ...
    void add_layout_transpose_case(int src_mem, int dst_mem, int rows, int cols, int seed) {
        ...
        const auto src_ref = alloc_matrix(c, "src", src_mem, rows, cols);
        const auto dst_ref = alloc_matrix(c, "dst", dst_mem, c.dst_rows, c.dst_cols);
        c.src_base = src_ref.word_addr;
        c.dst_base = dst_ref.word_addr;
        c.src_words = src_ref.word_count;
        c.dst_words = dst_ref.word_count;
        ...
        apply_instruction(c, runtime_.layout_transpose(src_ref.name, dst_ref.name));
    }
    ...
    void add_layout_assemble_case(int src_mem, int dst_mem, int src_rows, int src_cols,
        ...
        const auto src_ref = alloc_matrix(c, "src", src_mem, src_rows, src_cols);
        const auto dst_ref = alloc_matrix(c, "dst", dst_mem, c.dst_rows, c.dst_cols);
        c.src_base = src_ref.word_addr;
        c.dst_base = dst_ref.word_addr;
        c.src_words = src_ref.word_count;
        ...
        c.dst_words = pack_matrix_words(c.pre_dst, dst, c.dst_rows, c.dst_cols);
        apply_instruction(c, runtime_.layout_assemble(src_ref.name, dst_ref.name, c.off_r, c.off_c));
    }
    ...
    void add_reduce_cmp_case(int src_mem, int len, int min_idx, int seed) {
        ...
        c.src_words = pack_vector_words(c.pre_src, vec, len);
        apply_instruction(c, runtime_.reduce_cmp(src_ref.name));
    }
    ...
    void add_gemm_identity_case(int a_mem, int b_mem, int c_mem,
        ...
        const auto a_ref = alloc_matrix(c, "a", a_mem, n_rows, c.kdim);
        const auto b_ref = alloc_matrix(c, "b", b_mem, c.kdim, m_cols);
        const auto c_ref = alloc_matrix(c, "c", c_mem, n_rows, m_cols);
        c.src_base = a_ref.word_addr;
        c.aux_base = b_ref.word_addr;
        c.dst_base = c_ref.word_addr;
        c.src_words = a_ref.word_count;
        c.aux_words = b_ref.word_count;
        c.dst_words = c_ref.word_count;
        ...
        apply_instruction(c, runtime_.gemm(a_ref.name, b_ref.name, c_ref.name));
    }
    ...
    void add_mul_copy_case(int a_mem, int c_mem, int rows, int cols, int seed) {
        ...
        c.src_elems = rows * cols;
        c.dst_elems = rows * cols;
        const auto a_ref = alloc_matrix(c, "a", a_mem, rows, cols);
        const auto c_ref = alloc_matrix(c, "c", c_mem, rows, cols);
        c.src_base = a_ref.word_addr;
        c.dst_base = c_ref.word_addr;
        c.src_words = a_ref.word_count;
        c.dst_words = c_ref.word_count;
        ...
        apply_instruction(c, runtime_.mul(a_ref.name, c_ref.name, c.alpha));
    }
    ...
    void add_add_zero_case(int a_mem, int b_mem, int c_mem, int rows, int cols, int seed) {
        ...
        c.dst_elems = rows * cols;
        const auto a_ref = alloc_matrix(c, "a", a_mem, rows, cols);
        const auto b_ref = alloc_matrix(c, "b", b_mem, rows, cols);
        const auto c_ref = alloc_matrix(c, "c", c_mem, rows, cols);
        c.src_base = a_ref.word_addr;
        c.aux_base = b_ref.word_addr;
        c.dst_base = c_ref.word_addr;
        ...
        apply_instruction(c, runtime_.add(a_ref.name, b_ref.name, c_ref.name));
    }
    ...
    void preload_cases_to_sram() {
        for (int cid = 0; cid < num_cases_; ++cid) {
            ...
            if (c.dst_words > 0) {
                ...
                runtime_.write_memory(c.dst_mem, c.dst_base, word_slice(c.pre_dst, c.dst_words));
            }
            if (c.src_words > 0) {
                check_mem_addr(c.src_mem, c.src_base + c.src_words - 1);
                runtime_.write_memory(c.src_mem, c.src_base, word_slice(c.pre_src, c.src_words));
            }
            if (c.aux_words > 0) {
                check_mem_addr(c.aux_mem, c.aux_base + c.aux_words - 1);
                runtime_.write_memory(c.aux_mem, c.aux_base, word_slice(c.pre_aux, c.aux_words));
            }
        }
    }
    ...
    void run_device_issue_order() {
        ...
        for (int cid : reduce_ids) {
            ...
            std::cout << "Device mixed " << label_
                      ...
                      << " done_count_before=" << runtime_.read_status().done_count
                      << std::endl;
            runtime_.send_instruction(runtime_.with_group_end(c.instruction, group_end));
            const auto status = runtime_.wait_next_done(kTimeoutCycles);
        ...
        for (int cid : non_reduce_ids) {
            ...
            std::cout << "Device mixed " << label_
                      ...
                      << " group_end=" << (group_end ? 1 : 0)
                      << std::endl;
            auto instruction = runtime_.with_group_end(cases_[static_cast<std::size_t>(cid)].instruction, group_end);
            runtime_.send_instruction(instruction);
            ++issue_idx;
        }

        std::cout << "Device mixed " << label_
                  << " waiting done_count target=" << num_cases_
                  << " current=" << runtime_.read_status().done_count
                  << std::endl;
        const auto status = runtime_.wait_done_count(static_cast<std::uint32_t>(num_cases_), kTimeoutCycles);
    ...
    void capture_case_output(int cid) {
        ...
        clear_word_vec(c.post_dst);
        const auto words = runtime_.read_memory(c.dst_mem, c.dst_base, c.dst_words);
